...
class TestBucketList(BaseTestCase):
    ...
    def test_bucketlist_creation_requires_valid_authentication(self):
        ...
        invalid_token = self.client.post('/bucketlists/', data=dict(
            name='Expired Bucketlist Name'), headers={'token': self.invalid_token})
    ...
    def test_duplicate_bucketlist_creation_fails(self):
        ...
        duplicate_response = self.client.post('/bucketlists/', data=dict(
            name='First Bucketlist'), headers={'token': self.token})
        self.assertEqual(duplicate_response.json['message'], 'Error creating bucketlist')
    ...
    def test_get_single_bucketlist_requires_authentication(self):
        ...
        With an authentication token a 200 OK response should be returned.
        """
        no_auth = self.client.get('/bucketlists/{0}'.format(self.bl1.json['id']))
        get_bl = self.client.get('/bucketlists/{0}'.format(
            self.bl1.json['id']), headers={'token': self.token})
        not_found = self.client.get('/bucketlists/50', headers={'token': self.token})
        self.assert_401(no_auth)
        self.assert_200(get_bl)
    ...
    def test_del_bucketlists_requires_authentication(self):
        ...
        invalid_id = self.client.delete('/bucketlists/50', headers={'token': self.token})
        bl = Bucketlist.query.all()
    ...
    def test_bucketlist_pagination(self):
        ...
        get_max = self.client.get('/bucketlists/?limit=150', headers={'token': self.token})
    ...
    def test_search_by_name(self):
        ...
        search_err = self.client.get('/bucketlists/?q=tenth', headers={'token': self.token})
